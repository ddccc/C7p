...
// File: c:/bsd/rigel/sort/C7/ParC4.c
// Date: Sun Oct 20 17:15:14 2019
...
static char* expiration4 = "*** License for sixsort has expired ...\n";

// Here more global entities used throughout
// int (*compareXY)();
// void **A;

// inherited from elsewhere in this context:
...
void *sortThread4(void *AAA) { // AAA-argument is NOT used
  // int taskCnt = 0;
...
int cut2SLimit4 = 2000;
void c4p(void **A, int size, 
  ...
  */
  ...
  if ( size <= cut2SLimit4 || numberOfThreads <= 1 ) {
  ...
  if ( compareXY(T, A[N]) <= 0 || compareXY(A[M], T) < 0 ) {
    ...
    */
    ...
    if ( (errcode=pthread_create(&thread_id[1], NULL, 
      ...
      errexit(errcode,"ParC4/cp4()/pthread_create");
    }
    ...
    if ( (errcode=pthread_join(thread_id[1], NULL) )) {
      errexit(errcode,"ParC4/cp4()/pthread_join");
    }
