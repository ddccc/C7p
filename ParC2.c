// File: c:/bsd/rigel/sort/C7p/ParC2.c
// Date: Sat Oct 14 15:58:04 2023
...
// void cut2lrp(); // sequential version not used
void cut2lrpc();
...
void c2pc();
void c2p(void **A, int lo, int hi,  
	 int (*compareXY) (const void *, const void * ),
	 int numberOfThreads ) {
	 // int numberOfThreads) {
  int L = hi - lo;
  if ( L <= cut2S2Limit  || numberOfThreads <= 1 ) {
    // quicksort0(A, 0, size-1, compareXY);
    cut2lr(A, lo, hi, compareXY);
    return;
  }
  int depthLimit = 2.9 * floor(log(L));
  c2pc(A, lo, hi, depthLimit, compareXY, numberOfThreads);
}

void c2pc(void **A, int lo, int hi, int depthLimit,
	  int (*compareXY) (const void *, const void * ),
	  int numberOfThreads) {
  int L = hi-lo +1;
  // printf("c2pc %i %i \n ", L, numberOfThreads);
  ...
  for ( i = 0; i < NUMTHREADS; i++ ) {
    if ( errcode=pthread_create(&thread_id[i], NULL, 
				sortThread1, (void *)A ) ) {
      errexit(errcode,"ParC2/ c2p()/pthread_create");
    }
  }

  for ( i = 0; i < NUMTHREADS; i++ ) {
    if ( errcode=pthread_join(thread_id[i], NULL) ) {
      errexit(errcode,"ParC2/ c2p()/pthread_join");
    }
  }
  free(ll);
