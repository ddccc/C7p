...
const int cut4dLimit = 1600; 

void cut4dc();
// cut4dc is doing 4-partitioning using 3 pivots
...
void cut4dc(void **A, int N, int M, int depthLimit, int (*compareXY)()) {
  int L;
 Start:
  // printf("cut4dc %d %d \n", N, M);
  L = M - N +1; 
  ...
  if ( compareXY(maxl, middle) == 0 || 
    ...
    dflgm(A, N, M, middlex, cut4dc, depthLimit, compareXY);
    return;
  }
