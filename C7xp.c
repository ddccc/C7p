// c:/bsd/rigel/sort/C7p/C7xp.c
// Date: Fri Oct 20 17:19:38 2023
// (C) OntoOO/ Dennis de Champeaux


#define iswap(p, q, A) { void *t3t = A[p]; A[p] = A[q]; A[q] = t3t; }

int cut7Limit7 = 4000; // transition to 1-pivot

void cut7pc(void **A, int lo, int hi, int depthLimit,
	   int (*compare)(const void*, const void*));

void cut7p(void **A, int lo, int hi, int (*compare)()) {
  // printf("cut7p N %i lo %i \n", lo, hi);
  int L = hi - lo;
  if ( L <= 0 ) return;

  int depthLimit = 1 + 2.9 * floor(log(L));
  cut7pc(A, lo, hi, depthLimit, compare);
} // end cut7p

void cut7pc(void **A, int lo, int hi, int depthLimit,
		int (*compare)(const void*, const void*)) {
  int L;
  //   int i, j; // running indices
  // void *ai, *aj;

 Start:
  //  printf("cut7c lo %i hi %i L %i\n", lo, hi, hi-lo);
  L = hi - lo + 1;
  if ( L <= 1 ) return; 
  if ( depthLimit <= 0 ) {
    heapc(A, lo, hi, compare);
    return;
  }
  depthLimit--;

  if ( L < cut7Limit7 ) {
    // cut2lr2c(A, lo, hi, depthLimit, compare);
    // cut2Nk1nc(A, lo, hi, depthLimit, compare);
    // cut2k2c(A, lo, hi, depthLimit, compare);
    // cut2lrc(A, lo, hi, depthLimit, compare);
    cut7c(A, lo, hi, depthLimit, compare);
    return;
  }

 
  int k, lo1, hi1; // for sampling
  int maxlx, middlex, minrx;  
  // pivots for left/ middle / right regions
  register void *maxl, *middle, *minr;   
  register int i, j, lw, up, z; // indices
  i = lo; j = hi;
  z = middlex = lo + (L>>1); // lo + L/2/
  //const int small = 900; 
{ // small <= L, use a variable number for sampling
  // int probeLng = sqrt(L/5.8); 
    int probeLng = sqrt(L/5.6); 
    int halfSegmentLng = probeLng >> 1; // probeLng/2;
    int quartSegmentLng = probeLng >> 2; // probeLng/4;
    lo1 = middlex - halfSegmentLng; //  lo + (L>>1) - halfSegmentLng;
    hi1 = lo1 + probeLng - 1;
    maxlx = lo1 + quartSegmentLng;
    // int middlex = lo1 + halfSegmentLng;
    minrx = hi1 - quartSegmentLng;
    int offset = L/probeLng;  

    // assemble the mini array [lo1, hi1]
    for (k = 0; k < probeLng; k++) // iswap(lo1 + k, lo + k * offset, A);
    { int xx = lo1 + k, yy = lo + k * offset; iswap(xx, yy, A); }
    // sort this mini array to obtain good pivots
    cut7c(A, lo1, hi1, depthLimit, compare);
    lw = maxlx; up = minrx;
  }
  // pivots
  maxl = A[maxlx]; middle = A[z]; minr = A[minrx];

  // check that segments can be properly initialized
  if ( compare(maxl, middle) == 0 || 
       compare(middle, minr) == 0 || 
       compare(minr, A[hi1]) == 0 ||
       compare(maxl, A[lo1]) == 0 ) {
    // no good pivots available, thus escape
    dflgm(A, lo, hi, middlex, cut7c, depthLimit, compare);
    return;
  }
  //  if ( use5sample < L ) {
    // Swap these segments to the corners
    for ( k = lo1; k <= maxlx; k++ ) {
      iswap(k, i, A); i++;
    }
    i--;
    lw = i+1;
    for ( k = maxlx+1; k <= middlex; k++ ) {
      iswap(k, lw, A); lw++;
    }
    lw--;
    for ( k = hi1; minrx <= k; k--) {
      iswap(k, j, A); j--;
    }
    j++;
    up = j-1;
    for ( k = minrx-1; middlex < k; k--) {
      iswap(k, up, A); up--;
    }
    up++;
    // } 

    void *alw, *aup; // values  

    // Here the general layout
    /*  L     ML                        MR     R
     |-----]------]------------------[------[-----|
    lo     i     lw                  up     j     hi

     There are invariants to be maintained (which are >essential< 
     for machine assisted correctness proofs):
     maxl < middle < minr
       lo <= x <= i --> A[x] <= maxl
       i < x <= lw --> maxl < A[x] <= middle
       up <= x < j --> middle < A[x] < minr
       j <= x <= hi --> minr <= A[x]
   */
    // Ready to roll ... 

    while ( ++lw < up ) {
      alw = A[lw];
      while ( compare(alw, middle) <= 0 ) {
	if ( compare(alw, maxl) <= 0 ) {
	  A[lw] = A[++i]; A[i] = alw;
	}
	if ( up <= ++lw ) { lw = up-1; goto GapClosed; }
	alw = A[lw];
      }
      // alw -> right

      if ( --up <= lw ) { goto InsertALW; }
      aup = A[up];
      while ( compare(middle, aup) < 0 ) {
    	if ( compare(minr, aup) <= 0 ) {
	  A[up] = A[--j]; A[j] = aup;
	}
	if ( --up <= lw )  { goto InsertALW; }
	aup = A[up];
      }
      // aup -> left

      if ( compare(aup, maxl) <= 0 ) {
	A[lw] = A[++i]; A[i] = aup;
      } else A[lw] = aup; 
      if ( compare(minr, alw) <= 0 ) {
	A[up] = A[--j]; A[j] = alw;
      } else A[up] = alw;
     }

    // fall through
    lw--;
    goto GapClosed;

 InsertALW:
    if ( compare(minr, alw) <= 0 ) {
      A[up] = A[--j]; A[j] = alw;
    } else A[up] = alw;
    lw--;

 GapClosed: ;

  // +++++++++++++++ gap closed+++++++++++++++++++
  /*      L     ML                        MR     R
       |-----]------]------------------[------[-----|
      lo     i     lw                  up     j     hi
  */
  // int k; // for testing

  /*
    for ( k = lo; k <= i; k++ ) if ( compare(maxl, A[k]) < 0 ) {
	printf("Err0 L k: %i\n",k);
	exit(0);
      }
    for ( k = i+1; k <= lw; k++ ) 
      if ( compare(A[k], maxl) <= 0 || compare(middle, A[k]) < 0) {
	printf("Err0 ML k: %i\n",k);
	printf("lo %i i %i lw %i up %i j %i hi %i\n",lo,i,lw,up,j,hi);
	// printf("---- callCnt %i\n", callCnt);
	exit(0);
      }
    for ( k = up; k < j; k++ ) 
      if ( compare(A[k], middle) <= 0 || compare(minr, A[k]) <= 0) {
	struct intval *pk, *pmr;
	// pk = A[326327]; pmr = minr;
	// printf("pk %i minr %i\n", pk->val, pmr->val);
	// printf("Err0 MR k: %i callCnt %i \n",k, callCnt);
	printf("Err0 MR k: %i\n",k);
	printf("Err0 lo %i i %i lw %i up %i j %i hi %i\n",lo,i,lw,up,j,hi);
	exit(0);
      }
    for ( k = j; k <= hi; k++ ) if ( compare(A[k], minr) < 0 ) {
	printf("Err0 R k: %i\n",k);
	exit(0);
      }

    // printf("Rcall lo %i i %i lw %i up %i j %i hi %i\n",lo,i,lw,up,j,hi);
    // */

  /*      L     ML                        MR     R
       |-----]------------][------------------[-----|
      lo     i          lw up                 j     hi
  */
    /*
  if ( lw-lo < hi-lw ) {
    cut7c(A, lo, i, depthLimit, compare);
    cut7c(A, i+1, lw, depthLimit, compare);
    if ( j-up < hi-j ) {
      cut7c(A, up, j-1, depthLimit, compare);
      // cut7c(A, j, hi, depthLimit, compare);
      // return;
      lo = j;
      goto Start;
    }
    cut7c(A, j, hi, depthLimit, compare);
    // cut7c(A, z+1, j-1, depthLimit, compare);
    // return;
    lo = up; hi = j-1;
    goto Start;
  }
  cut7c(A, up, j-1, depthLimit, compare);
  cut7c(A, j, hi, depthLimit, compare);
  if ( i-lo < lw-i ) {
    cut7c(A, lo, i, depthLimit, compare);
    // cut7c(A, i+1, lw, depthLimit, compare);
    // return;
    lo = i+1; hi = lw;
    goto Start;
  }
  cut7c(A, i+1, lw, depthLimit, compare);
  // cut7c(A, lo, i, depthLimit, compare);
  hi = i;
  goto Start;
    */
  /*      L     ML                        MR     R
       |-----]------------][------------------[-----|
      lo     i          lw up                 j     hi
  */

  if ( lw-lo < hi-lw ) {
    if ( j-up < hi-j ) {
      // cut7c(A, j, hi, depthLimit, compare);
      addTaskSynchronized(ll, newTask(A, j, hi, depthLimit, compare));
      // push cut7c(A, up, j-1, depthLimit, compare);
      addTaskSynchronized(ll, newTask(A, up, j-1, depthLimit, compare));
    } else {
      // push cut7c(A, up, j-1, depthLimit, compare);
      addTaskSynchronized(ll, newTask(A, up, j-1, depthLimit, compare));
      //push cut7c(A, j, hi, depthLimit, compare);
      addTaskSynchronized(ll, newTask(A, j, hi, depthLimit, compare));
    }
    // push cut7c(A, i+1, lw, depthLimit, compare);
    addTaskSynchronized(ll, newTask(A, i+1, lw, depthLimit, compare));
    hi = i;
    goto Start;
  }
  if ( i-lo < lw-i ) {
    // push cut7c(A, i+1, lw, depthLimit, compare); 
    addTaskSynchronized(ll, newTask(A, i+1, lw, depthLimit, compare));
    // push cut7c(A, lo, i, depthLimit, compare);
    addTaskSynchronized(ll, newTask(A, lo, i, depthLimit, compare));
  } else {
    // push cut7c(A, lo, i, depthLimit, compare);
    addTaskSynchronized(ll, newTask(A, lo, i, depthLimit, compare));
    // push cut7c(A, i+1, lw, depthLimit, compare); 
    addTaskSynchronized(ll, newTask(A, i+1, lw, depthLimit, compare));
  }
  // push cut7c(A, j, hi, depthLimit, compare);
  addTaskSynchronized(ll, newTask(A, j, hi, depthLimit, compare));
  lo = up; hi = j-1;
  goto Start;

} // end 3-pivot module cut7pc

#undef iswap
